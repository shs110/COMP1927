...
{
    assert(start > NOWHERE && start <= ZURICH);
    assert(end > NOWHERE && end <= ZURICH);
    assert(type > NONE && type < ANY);

    VNode newNode1 = malloc(sizeof(VNode));
    newNode1->v = end;
    newNode1->type = type;
    newNode1->next = g->connections[start];
    g->connections[start] = newNode1;

    VNode newNode2 = malloc(sizeof(VNode));
    newNode2->v = start;
    newNode2->type = type;
    newNode2->next = g->connections[end];
    g->connections[end] = newNode2;    
