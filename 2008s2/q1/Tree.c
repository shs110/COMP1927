...
// read items for tree in level-order and build tree
Tree scan(FILE *inf)
{
	char values[MAXNODES]; // assume no Trees > 1000 Nodes
	Node *nodes[MAXNODES];
	int  val, i, p, n;
	...
	n = 0;
	while ((val = fgetc(inf)) != EOF)
		if (!isspace(val)) values[n++] = val;

	if (n == 0) return NULL;
	nodes[0] = newNode(values[0]);
	for (i = 1; i < n; i++) {
