...
{
    printf("IM RUNNING RIHGT NOW INSERTINORDER!!!\n");
    struct IntListNode *current;
    struct IntListNode *nodePointer = malloc(sizeof(struct IntListNode));

    current = L->first;
    assert(L != NULL);
	nodePointer->data = v;
	
    printf("Passed Assertion1\n");
    if (current == NULL){
        printf("If STARTING!!!\n");
        current = nodePointer;
		...
		while (current->next != NULL){
			...
			if (v > current->data){   
				...
				nodePointer->next = current->next->next;   
